...
# ============================================================================
# Configuration parameters for high accuracy with vanilla scVI DE
# ============================================================================
MAX_EPOCHS = 200                   # Increased epochs for better convergence
BATCH_SIZE = 1024                  # Balanced batch size for large models
LATENT_DIM = 30                   # Larger latent space to capture complex biology
HIDDEN_UNITS = 256                # More hidden units for complex representations
HIDDEN_LAYERS = 3                 # Deeper network (3 layers)
DROPOUT_RATE = 0.15               # Moderate dropout to avoid overfitting
USE_AMPMIXED16 = True
CONVERT_TO_DENSE = True           # Use dense if needed for performance
NUM_DL_WORKERS = min(4, os.cpu_count() or 1)
MIN_CELLS_PER_GENE_FRAC = 0.10

DE_N_SAMPLES = 2000               # More DE posterior samples for precision
DE_ALL_STATS = True               # Collect full posterior stats
MC_N_SAMPLES = 500                # More normalization samples
...
OUTPUT_DIR = "/20TB-storage/aditya22598/zoo/output_ultrafast"
...
def run_cell_type(ct: str):
    try:
        ...
        de = model.differential_expression(
            ...
            mode="vanilla",                # Vanilla mode retained
